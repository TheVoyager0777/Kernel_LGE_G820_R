				      ...
				      QDF_STATUS *ret_status)
{
	...
	/* If SAP is not UP don't proceed further */
	...
	if (is_ch_dfs && sap_ctx->pre_cac_complete) {
		QDF_TRACE(QDF_MODULE_ID_SAP, QDF_TRACE_LEVEL_INFO_MED, FL(
		    "sapdfs: SAP_STARTED => SAP_STARTING, on pre cac"));
		...
		if ((false == mac_ctx->sap.SapDfsInfo.ignore_cac)
			...
			QDF_TRACE(QDF_MODULE_ID_SAP, QDF_TRACE_LEVEL_INFO_MED,
				  "%s: %d: sapdfs: from state SAP_STARTED => SAP_INIT with ignore cac false on sapctx[%pK]",
			...
			QDF_TRACE(QDF_MODULE_ID_SAP,
				  ...
				  "%s: %d: sapdfs: from state SAP_STARTED => SAP_STARTING with ignore cac true on sapctx[%pK]",
				  __func__, __LINE__, sap_ctx);
		...
		QDF_TRACE(QDF_MODULE_ID_SAP, QDF_TRACE_LEVEL_INFO_MED,
			  "%s: %d: sapdfs: from state SAP_STARTED => SAP_STARTING on sapctx[%pK]",
			  __func__, __LINE__, sap_ctx);
		/* non-DFS channel */
		sap_ctx->fsm_state = SAP_STARTING;
		mac_ctx->sap.SapDfsInfo.sap_radar_found_status = false;
